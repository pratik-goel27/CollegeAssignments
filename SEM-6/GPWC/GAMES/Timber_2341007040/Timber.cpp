...
int main() {
	...
	while (window.isOpen()) {
		...
		if(!paused){
            ...
            timeBar.setSize(Vector2f(timeBarWidthPerSecond * timeRemaining, timeBarHeight)); //set height and width of time bar.
            if(timeRemaining <= 0) {
            	paused = true;
			...
			if(!beeActive){
				...
				srand((int)time(0)*10);
				float height=(rand()%500)+500; //to set the height 500-999
			...
			if(logActive){
				...
				if(spriteLog.getPosition().x < -100 || spriteLog.getPosition().x > 2000) {  //if the log goes out of screen on either left or right
					logActive = false;
					spriteLog.setPosition(810,720);  //log is reseted to it's default place.
				}
			}
